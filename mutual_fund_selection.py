...
st.markdown(f"Download the holdings CSVs for these schemes and place them in the folder: fundata (create it if it doesn't exist).")
...
# Create a separate multi-select for overlap analysis
overlap_selection = st.multiselect(
    "Select schemes for overlap analysis:",
    options=list(df['SCHEMES']),
...
if st.button("Show Overlap Heatmap (after placing CSVs)"):
    if fund_files:
        # # Build a mapping: filename → DataFrame (for uploaded files)
        # fund_dfs = {}
        # unique_stocks = set()
        ...
        sns.heatmap(corr_mf, annot=True, cmap='YlGnBu', fmt='.2f', ax=ax, annot_kws={"size": 12}, linecolor= "black")
        st.pyplot(fig)
